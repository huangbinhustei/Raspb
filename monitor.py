...
def single_line_maker(start, end):
    ...
    line_cpu = Line(width=640, height=360, title='CPU 监控')
...
def group_line_maker(start, end, group_by):
    ...
    def lining_grouped():
        ...
        return stamps[1:], top_list, bottom_list

    line_cpu = Line(width=640, height=360, title='CPU 监控')
    line_space = Line(width=640, height=360, title='DISK 监控')
    stamps, [cpu_percents_top, cpu_temps_top, free_rams_top, free_disks_top], [cpu_percents_bottom, cpu_temps_bottom,
                                                                               free_rams_bottom,
    ...
    line_space.add('剩余磁盘', stamps, free_disks_bottom, is_smooth=True)
